...
mongoose.connect('mongodb://127.0.0.1:27017/objects', {
...
router.post('/api/image', upload.single('photo'), (req, res) => {
    res.send({
...
router.put('/api/objects/image/:object', function(req, res, next) {
    req.object.imageChange(req.body.imageName, function(err, object) {
        if (err) { return next(err); }
        res.json(object);
    });
});
router.put('/api/objects/title/:object', function(req, res, next) {
    req.object.titleChange(req.body.title, function(err, object) {
        if (err) { return next(err); }
        res.json(object);
    });
});
router.put('/api/objects/text/:object', function(req, res, next) {
    req.object.textChange(req.body.text, function(err, object) {
        if (err) { return next(err); }
        res.json(object);
    });
});
router.delete('/api/objects/:id', function(req, res, next) {
    try {
...
router.get('/api/objects/:object', function(req, res) {
    res.json(req.object);
});
